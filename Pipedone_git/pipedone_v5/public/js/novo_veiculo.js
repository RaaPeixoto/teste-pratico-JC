...
async function setProdutosHinova() {
    ...
    for (produto of produtos) {
        ...
        if (produto.tipo_veiculo == tipo_veiculo || produto.tipo_veiculo == "0") {
            ...
            // console.log(produto.formato_cobranca)
...
function gerarProposta() {
    ...
    axios.post(`/api/proposals/create`, {
        ...
        if (resposta.status == 200) {
            ...
            var idEnconde = btoa(resposta.data.id);

            var msg = "";
            msg += `Olá ${proposals.client.split(' ')[0]}!`;
            msg += `<br>`;
            msg += `<br>`;
            msg += `Segue abaixo a proposta de proteção veicular que solicitou:`;
            msg += `<br>`;
            msg += `<br>`;
            msg += `<a href="${url}/${idEnconde}">Clique para ver a proposta</a>`;
            msg += `<br>`;
            msg += `<br>`;
            msg += `Ou acesse o link:`;
            msg += `${url}/${idEnconde}`;
            msg += `<br>`;

            var to = proposals.email;
            var subject = `Você recebeu uma proposta de proteção veicular`;
